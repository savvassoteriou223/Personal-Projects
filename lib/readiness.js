...
export function adjustSessionForReadiness(sets, label) {
  ...
  return sets.map(ex => {
    ...
    // The UI renders an ASCENDING ramp (early_rpe → last_rpe: start easier,
    // finish harder). Lowering last_rpe alone can invert that ramp (e.g. the
    // library's dominant 9→10 case would become 9→8). So early_rpe must drop
    // too, and stay clamped to never exceed the new last_rpe.
    const nextLast = Math.max(RPE_FLOOR, (ex.last_rpe ?? 9) - 2);
    const nextEarly = Math.min(nextLast, Math.max(RPE_FLOOR, (ex.early_rpe ?? 7) - 2));
    return {
      ...ex,
      early_rpe: nextEarly,
      last_rpe: nextLast,
