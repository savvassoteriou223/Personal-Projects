...
/**
 * Share a transparent sticker rather than a card.
 *
 * Two things differ from `shareCard` and both matter:
 *
 * 1. `backgroundColor: 'transparent'` — without it view-shot composites onto
 *    white and the sticker arrives as a white slab.
 * 2. No width/height. `shareCard` pins its output size so the same card exports
 *    identically everywhere, but a sticker must be cropped TIGHT to its own
 *    bounds: hand Instagram a story-sized canvas with a mark in one corner and
 *    it treats the whole canvas as the sticker, which then drags as a
 *    screen-sized object and cannot be placed. Capturing the sticker view at
 *    its natural size is what makes it behave like a real sticker.
 */
export async function shareSticker(viewRef, fallbackMessage) {
  if (await canShareImage()) {
    try {
      const uri = await captureRef(viewRef, {
        format: 'png',
      ...
      console.warn('shareSticker: capture failed, falling back to text', err);
    }
  }
