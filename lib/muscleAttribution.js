...
export function normaliseMuscle(raw) {
  ...
  // Forearms are trained deliberately (wrist curls, carries, Zottman curls) and
  // TodayScreen's own normaliser has always mapped them, so leaving them out
  // here meant the recovery map and the coach could not see work the volume
  // screen could. They have no VOLUME_TARGETS row, which is why they are absent
  // from MUSCLE_GROUPS — that is about targets, not about attribution.
  if (['forearms', 'brachioradialis', 'wrist flexors', 'wrist extensors'].includes(r)) return 'forearms';
  return null; // genuinely untracked (e.g. adductors)
}
